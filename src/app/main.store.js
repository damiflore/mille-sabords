...
import { createStructuredStateStore } from "/app/store/createStructuredStateStore.jsx"
import { cardIds } from "/app/cards/cards.js"
  ...
  () => {
    if (localStorage.hasOwnProperty(stateStorageKey)) {
      ...
      const missingKey = Object.keys(defaultState).find(
        (key) => key in valueFromStorage === false,
      )
      if (missingKey) {
        logger.warn(
      ...
      const extraKey = Object.keys(valueFromStorage).find(
...
export const useCurrentCardActivated = () =>
  store.useKeyedState("currentCardActivated")
...
export const useWitchUncursedDiceId = () =>
  store.useKeyedState("witchUncursedDiceId")
export const useWitchCardEffectUsed = () =>
  store.useKeyedState("witchCardEffectUsed")
...
export const useAnimationsDisabled = () =>
  store.useKeyedState("animationsDisabled")
